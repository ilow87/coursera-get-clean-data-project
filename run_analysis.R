run_analysis <- function(){
  ...
  ## Comining test and training dataset to form 1 single data table
  ...
  data <- merge(label.y, data, by="activity_code")
  
  
  ## applying cleaned variable names to data tables
  names(data) <- c("activity_code", "activity", "subject_code", data.cols.name)
  
  
  library(dplyr)
  
  ## summarising data table to show the mean of all variables grouped against activity & subject_code
  summary <-  data %>% 
  ...
  ## writing summary table to a txt file in working directory
